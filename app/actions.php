...
try {
    switch ($action) {
        ...
        case 'contact_submit':
            ...
            if (mb_strlen($name) < 2 || !filter_var($email,
                    FILTER_VALIDATE_EMAIL) || mb_strlen($subject) < 3 || mb_strlen($message) < 10) {
            ...
            flash('success',
                $sent ? 'Cảm ơn bạn! Tin nhắn đã được gửi tới đội ngũ Vi Vu Việt.' : 'Cảm ơn bạn! Tin nhắn đã được ghi nhận, đội ngũ sẽ phản hồi sớm.');
        ...
        case 'toggle_like':
            ...
            if ($stmt->fetchColumn()) {
                $pdo->prepare('DELETE FROM article_likes WHERE article_id=? AND user_id=?')->execute([
                    $articleId,
                    $user['id']
                ]);
                flash('success', 'Đã bỏ bài viết khỏi danh sách yêu thích.');
            } else {
                $pdo->prepare('INSERT INTO article_likes (article_id,user_id) VALUES (?,?)')->execute([
                    $articleId,
                    $user['id']
                ]);
        ...
        case 'rate_article':
            ...
            $pdo->prepare('INSERT INTO ratings (article_id,user_id,score) VALUES (?,?,?) ON DUPLICATE KEY UPDATE score=VALUES(score), updated_at=CURRENT_TIMESTAMP')->execute([
                $articleId,
                $user['id'],
                $score
            ]);
        ...
        case 'add_comment':
            ...
            $pdo->prepare('INSERT INTO comments (article_id,user_id,content) VALUES (?,?,?)')->execute([
                $articleId,
                $user['id'],
                $content
            ]);
            ...
            if ($placeId) {
                ...
                $pdo->prepare("UPDATE places SET province_id=?,name=?,category_id=?,address=?,short_description=?,description=?,cover_image=?,price_info=?,opening_hours=?,status='pending',admin_note=NULL WHERE id=?")
                    ->execute([
                        ...
                        $description,
                        $cover,
                        trim((string) ($_POST['price_info'] ?? '')),
                        trim((string) ($_POST['opening_hours'] ?? '')),
                        $placeId
                    ]);
                ...
                flash('success',
                    $status === 'approved' ? 'Đã thêm và xuất bản địa điểm.' : 'Đã gửi địa điểm. Admin sẽ xem xét sớm nhất có thể.');
            }
            redirect($action === 'admin_submit_place' ? admin_url('dashboard', ['tab' => 'places']) : url('profile',
                ['tab' => 'submissions']));
        ...
        case 'moderate_place':
            ...
            $pdo->prepare('UPDATE places SET status=?,admin_note=?,approved_at=? WHERE id=?')->execute([
                $status,
                $note ?: null,
                $status === 'approved' ? date('Y-m-d H:i:s') : null,
                $placeId
            ...
            if ($place['submitted_by']) {
                ...
                $pdo->prepare('INSERT INTO notifications (user_id,title,message,link) VALUES (?,?,?,?)')->execute([
                    $place['submitted_by'],
                    'Cập nhật địa điểm: ' . $place['name'],
                    $message . ($note ? ' Ghi chú: ' . $note : ''),
                    url('profile', ['tab' => 'submissions'])
                ]);
            }
            $pdo->prepare('INSERT INTO moderation_logs (admin_id,place_id,action,note) VALUES (?,?,?,?)')->execute([
                $admin['id'],
                $placeId,
                $status,
                $note ?: null
        ...
        case 'admin_article_status':
            ...
            $pdo->prepare('UPDATE articles SET status=?,published_at=IF(?="published",COALESCE(published_at,NOW()),published_at) WHERE id=?')->execute([
                $status,
                $status,
                $articleId
            ]);
